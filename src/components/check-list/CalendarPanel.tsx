...
import { ChecklistEvent } from "@/fetchs/checkList.fetch";
...
const CalendarPanel = ({
    ...
    const eventMap = useMemo(() => {
        return events.reduce<Record<string, ChecklistEvent[]>>((acc, event) => {
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <CardTitle className="text-2xl font-bold tracking-tight">
                            {t("checkList.calendar.title")}
                        </CardTitle>
                        <CardDescription>{t("checkList.calendar.description")}</CardDescription>
                    </div>
                    <Badge variant="secondary" className="w-fit bg-primary/10 text-primary">
                        {t("checkList.calendar.friendBadge")}
                    </Badge>
                </div>
            </CardHeader>
                    ...
                    <div className="flex items-center justify-between gap-3">
                        ...
                        <div className="text-center">
                            <p className="text-sm uppercase tracking-wider text-muted-foreground">
                                {t("checkList.calendar.currentMonth")}
                            </p>
                    ...
                    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                        <DialogTrigger asChild>
                            <Button className="w-full gap-2">
                                ...
                                {t("checkList.calendar.addEvent")}
                            </Button>
                        </DialogTrigger>
                        <DialogContent className="max-w-lg">
                            <DialogHeader>
                                <DialogTitle>{t("checkList.calendar.dialog.title")}</DialogTitle>
                            </DialogHeader>
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    <label className="text-sm font-medium text-foreground">
                                        {t("checkList.calendar.dialog.titleLabel")}
                                    </label>
                                    <Input
                                        value={title}
                                        onChange={(event) => setTitle(event.target.value)}
                                        placeholder={t("checkList.calendar.dialog.titlePlaceholder")}
                                    />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-sm font-medium text-foreground">
                                        {t("checkList.calendar.dialog.friendLabel")}
                                    </label>
                                    <textarea
                                        value={friends}
                                        ...
                                        placeholder={t("checkList.calendar.dialog.friendPlaceholder")}
                                    />
                                    <p className="text-xs text-muted-foreground">
                                        {t("checkList.calendar.dialog.friendHelp")}
                                    </p>
                                </div>
                                <div className="space-y-2">
                                    <label className="text-sm font-medium text-foreground">
                                        {t("checkList.calendar.dialog.memoLabel")}
                                    </label>
                                    <textarea
                                        value={memo}
                                        onChange={(event) => setMemo(event.target.value)}
                                        className="min-h-[80px] w-full rounded-lg border border-border bg-background px-3 py-2 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"
                                        placeholder={t("checkList.calendar.dialog.memoPlaceholder")}
                                    />
                                </div>
                            </div>
                            <DialogFooter>
                                ...
                                >
                                    {isSubmitting
                                        ? t("checkList.calendar.dialog.saving")
                                        : t("checkList.calendar.dialog.save")}
                                </Button>
                            </DialogFooter>
                        </DialogContent>
                    </Dialog>
